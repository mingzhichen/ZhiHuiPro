...
namespace JieYiGuang.Common
{
    ...
    public class ImageCutHelper
    {
        ...
        /// <param name="quality">质量（范围0-100）</param>
        public static void CutForCustom(string originalImagePath, string fileSaveUrl, int maxWidth, int maxHeight, int quality)
        {
            //从文件获取原始图片，并使用流中嵌入的颜色管理信息
            //System.Drawing.Image initImage = System.Drawing.Image.FromStream(fromFile, true);
            Image initImage = Image.FromFile(originalImagePath, true);
            ...
            else
            {
                ...
                else
                {
                    ...
                    //保存缩略图
                    templateImage.Save(fileSaveUrl, GetCodecInfo("image/" + GetFormat(fileSaveUrl).ToString().ToLower()), ep);
                    //templateImage.Save(fileSaveUrl, System.Drawing.Imaging.ImageFormat.Jpeg);
        ...
        public static void ZoomAuto(string originalImagePath, string savePath, System.Double targetWidth, System.Double targetHeight, string watermarkText, string watermarkImage)
        {
            //创建目录
