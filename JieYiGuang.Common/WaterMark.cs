...
namespace JieYiGuang.Common
{
    public class WaterMark
    {
        ...
        {
            if(!File.Exists(UtilHelper.GetMapPath(imgPath)))
                return;
            byte[] _ImageBytes = File.ReadAllBytes(UtilHelper.GetMapPath(imgPath));
            Image img = Image.FromStream(new System.IO.MemoryStream(_ImageBytes));
            filename = UtilHelper.GetMapPath(filename);

            if (watermarkFilename.StartsWith("/") == false)
                watermarkFilename = "/" + watermarkFilename;
            watermarkFilename = UtilHelper.GetMapPath(watermarkFilename);
