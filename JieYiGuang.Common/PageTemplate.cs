...
namespace JieYiGuang.Common
{
    ...
    public abstract class PageTemplate
    {
        ...
        public static string GetTemplate(string sitePath, string tempPath, string skinName, string templet, string fromPage, string inherit, string buildPath, string channelName, string pageSize, int nest)
        {
            ...
            string templetFullPath = UtilHelper.GetMapPath(string.Format("{0}{1}/{2}/{3}", sitePath, tempPath, skinName, templet)); //取得模板文件物理路径
            ...
            using (StreamReader objReader = new StreamReader(templetFullPath, Encoding.UTF8))
            {
                ...
                //开始查找替换标签
                string[] strlist = UtilHelper.SplitString(textOutput.ToString(), "\r\r\n");
                ...
                if (nest == 1)
                {
                    ...
                    if (pageSize != string.Empty && UtilHelper.StrToInt(pageSize, 0) > 0)
                    {
                    ...
                    string template = string.Format("<%@ Page Language=\"C#\" AutoEventWireup=\"true\" Inherits=\"{0}\" ValidateRequest=\"false\" %>\r\n" +
                        ...
                        "<%@ Import namespace=\"JieYiGuang.Common\" %>{1}\r\n\r\n" +
                        "<script runat=\"server\">\r\noverride protected void OnInit(EventArgs e)\r\n" +
                        "{{\r\n\r\n\t/* \r\n\t\tThis page was created by JieYiGuang Template Engine at {2}.\r\n\t\t" +
                        "本页面代码由JieYiGuang模板引擎生成于 {2}. \r\n\t*/\r\n\r\n\tbase.OnInit(e);\r\n\t" +
                    ...
                    string pageDir = UtilHelper.GetMapPath(string.Format("{0}aspx/{1}/", sitePath, buildPath)); //生成文件的目录路径
        ...
        private static string ConvertTags(int nest, string channelName, string pageSize, string sitePath, string tempPath, string skinName, string inputStr)
        {
            ...
            foreach (Match m in r[15].Matches(strTemplate))
            {
                IsCodeLine = true;
                strTemplate = strTemplate.Replace(m.Groups[0].ToString(),
                              string.Format("\r\n\ttemplateBuilder.Append(UtilHelper.DropHTML({0},{1}));", m.Groups[2], m.Groups[3].ToString().Trim()));
            }
            #endregion
            ...
            foreach (Match m in r[20].Matches(strTemplate))
            {
                IsCodeLine = true;
                strTemplate = strTemplate.Replace(m.Groups[0].ToString(),
                              string.Format("\ttemplateBuilder.Append(UtilHelper.ObjectToDateTime({0}).ToString(\"{1}\"));", m.Groups[2], m.Groups[3].ToString().Replace("\\\"", string.Empty)));
            }
            #endregion
            ...
            foreach (Match m in r[21].Matches(strTemplate))
            {
                IsCodeLine = true;
                strTemplate = strTemplate.Replace(m.Groups[0].ToString(),
                    "UtilHelper.StrToInt(" + m.Groups[2] + ", 0)");
            }
            #endregion
            ...
            foreach (Match m in r[18].Matches(strTemplate))
            {
                if (IsCodeLine)
                {
                    if (UtilHelper.IsNumeric(m.Groups[3].ToString()))
                        strTemplate = strTemplate.Replace(m.Groups[0].ToString(), "UtilHelper.ObjectToStr(" + m.Groups[2] + "[" + m.Groups[3] + "])");
                    else
                    {
                ...
                {
                    if (UtilHelper.IsNumeric(m.Groups[3].ToString()))
                        strTemplate = strTemplate.Replace(m.Groups[0].ToString(), string.Format("\" + UtilHelper.ObjectToStr({0}[{1}]) + \"", m.Groups[2], m.Groups[3]));
                    else
                    {
                        if (m.Groups[3].ToString() == "_id")
            ...
            foreach (Match m in r[19].Matches(strTemplate))
            {
                if (IsCodeLine)
                    ...
                    strTemplate = strTemplate.Replace(m.Groups[0].ToString(),
                        string.Format("\");\r\n\ttemplateBuilder.Append(UtilHelper.ObjectToStr({0}));\r\n\ttemplateBuilder.Append(\"", m.Groups[2].ToString().Trim()));
            }
            #endregion
